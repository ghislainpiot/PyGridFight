...
2. **Scope** (MVP as defined in PRD, focusing on server-side architecture)
3. **Definitions, Acronyms, and Abbreviations**
4. **References** (Link to PRD: [`documents/PyGridFight_PRD.md`](documents/PyGridFight_PRD.md))
...
```mermaid
graph TD
    subgraph GameLifecycle
      GL[GameLifecycle Context] -->|Creates/Manages| GP[Gameplay Context]
    ...
    subgraph GameLifecycle_Context
...
- `/ws` (Primary endpoint for players and spectators)
...
- JSON (as per PRD [`documents/PyGridFight_PRD.md:147`](documents/PyGridFight_PRD.md:147))
...
- Server authoritative. Full state pushed after each validated action.
...
1. **GameSession Objects:** In-memory Python objects, each representing an active game.
2. **Tracking Active Games:** A central dictionary (e.g., `active_games: Dict[str, GameSession]`) managed by the `GameLifecycle` context.
3. **Player to Game Mapping:** How WebSocket connections are associated with `Player` objects within a `GameSession`.
4. **Game Lifecycle:** Creation (on demand or by player action), active state, termination (e.g., victory, all players leave).
...
1. **Unit Testing:** Focus on testing individual classes and methods within each Bounded Context (e.g., `Avatar.move()`, `ScoreKeeper.add_points()`, `VictoryConditionChecker.check_victory()`).
2. **Integration Testing:** Testing interactions, e.g., `GameSession.process_player_action()` correctly updates `Player` state, `Grid` state, and `ScoreKeeper`.
3. **API Testing:** Testing WebSocket communication, message validation, and state updates.

## 9. Deployment View (MVP)

1. **Single Process:** The FastAPI application runs as a single Python process.
2. **Scalability Considerations (Brief):** Mention that for MVP, it's single-instance; future scaling might involve multiple instances and a message bus/shared state if persistence is added.
