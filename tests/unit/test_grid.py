...
from src.pygridfight.domain.models.grid import Position, Grid
...
class TestGrid:
    ...
    @pytest.mark.parametrize("grid_size, pos, expected", [
    ...
    @pytest.mark.parametrize("grid_size, pos, expected", [
        ...
        # Corner (0,0)
        ((3, 3), Position(x=0, y=0), [
            Position(x=1, y=0),
            Position(x=0, y=1),
        ]),
        # Edge (0,1)
        ((3, 3), Position(x=0, y=1), [
            Position(x=1, y=1),
            Position(x=0, y=0),
            Position(x=0, y=2),
        ]),
        # Edge (2,1)
        ((3, 3), Position(x=2, y=1), [
            Position(x=1, y=1),
            Position(x=2, y=0),
            Position(x=2, y=2),
        ]),
