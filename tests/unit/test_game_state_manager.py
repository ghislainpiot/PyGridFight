...
@pytest.fixture
def game_settings():
    return GameSettings(grid_size=6, max_players=2, max_avatars_per_player=1, avatar_cost=3, target_score=10, max_turns=20)
...
@pytest.mark.anyio
async def test_player_connection_tracking(game_id, player_id, connection_info, game_settings):
    GameStateManager().reset()
    mgr = GameStateManager()
    await mgr.create_game(game_id, game_settings)
