...
class PlayerStats(BaseModel):
    ...
    total_score: int = Field(
        default=0, ge=0, description="Total score across all games"
    )
    average_score: float = Field(
        default=0.0, ge=0, description="Average score per game"
    )
...
class PlayerProfile(BaseModel):
    ...
    status: PlayerStatus = Field(
        default=PlayerStatus.OFFLINE, description="Current status"
    )
    ...
    stats: PlayerStats = Field(
        default_factory=PlayerStats, description="Player statistics"
    )
...
class PlayerUpdateRequest(BaseModel):
    ...
    name: str | None = Field(
        None, min_length=1, max_length=50, description="New player name"
    )
