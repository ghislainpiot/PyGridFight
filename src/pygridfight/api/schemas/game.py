...
class GameCreateRequest(BaseModel):
    ...
    max_players: int = Field(
        default=4, ge=2, le=8, description="Maximum number of players"
    )
...
class GameJoinRequest(BaseModel):
    ...
    player_name: str = Field(
        ..., min_length=1, max_length=50, description="Player name"
    )
...
class GameInfo(BaseModel):
    ...
    started_at: datetime | None = Field(None, description="When game started")
    finished_at: datetime | None = Field(None, description="When game finished")
...
class GameDetails(GameInfo):
    ...
    current_turn: str | None = Field(None, description="Current player's turn")
